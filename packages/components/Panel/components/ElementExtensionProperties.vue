<template>
  <el-collapse-item name="element-extension-properties">
    <template #title>
      <collapse-title title="Extended Attributes">
        <lucide-icon name="FileCog" />
      </collapse-title>
      <number-tag :value="extensions.length" margin-left="12px" />
    </template>
    <div class="element-extension-properties">
      <el-table border :data="extensions" style="width: 100%" height="200px" :empty-text="'no data'">
        ...
        <el-table-column label="Action" width="140">
          <template slot-scope="{ $index }">
            <el-button type="text" @click="removeProperty($index)">Remove</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: { required: true, message: "Attribute name cannot be empty", trigger: ["blur", "change"] },
        value: { required: true, message: "Attribute value cannot be empty", trigger: ["blur", "change"] }
      },
      modelVisible: false
    };
  },
